...
function json(body: unknown, status = 200): Response {
  return Response.json(body, { status, headers: { 'Cache-Control': 'no-store', 'Access-Control-Allow-Origin': 'https://foxprof.club', 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type' } });
}
...
export async function OPTIONS(): Promise<Response> { return json({ ok:true }); }
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (!LANGUAGES.has(sourceLanguage) || !LANGUAGES.has(targetLanguage) || sourceLanguage === targetLanguage) return json({error:'Unsupported or identical language selection'},400);
    const bytes = new Uint8Array(await audio.arrayBuffer()); if (!bytes.length) return json({error:'Audio file is empty'},400);
    if (bytes.length > 4 * 1024 * 1024) return json({error:'Audio file is too large (maximum 4 MB)'},413);
    ...
    const transcript = firstText(transcription); if (!transcript) return json({error:'No speech detected'},422);
    ...
    return json({transcript,translatedText,sourceLanguage,targetLanguage,...(audioBase64?{audioBase64,audioMimeType}:{})});
  } catch(error) { console.error('translate-voice failed',error); return json({error:`Translation service failed: ${error instanceof Error ? error.message : 'Unknown error'}`},500); }
}

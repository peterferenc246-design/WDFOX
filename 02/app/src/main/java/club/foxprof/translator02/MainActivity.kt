    ...
    MaterialTheme {
        Surface(Modifier.fillMaxSize()) {
            Column(
                            ...
                            if (file != null) {
                                ...
                                scope.launch {
                                    try {
                                        ...
                                        translation = result.second
                                        status = "Preklad hotový"
                                        result.third?.let { playAudio(it) }
                                    } catch (e: Exception) {
                ...
                Text("MVP: ${source.label} ↔ ${target.label}. Hlas → STT → AI preklad → TTS.")
            }
        }
    }
}
...
private suspend fun translateVoice(file: File, source: String, target: String): VoiceResult = withContext(Dispatchers.IO) {
    ...
    client.newCall(request).execute().use { response ->
        ...
        return@withContext VoiceResult(
...
private fun playAudio(audio: ByteArray) {
    ...
    val temp = File.createTempFile("fox02_tts_", ".mp3")
